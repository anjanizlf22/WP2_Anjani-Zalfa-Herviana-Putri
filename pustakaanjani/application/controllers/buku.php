...
class Buku extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->ModelBuku->simpanBuku($data);redirect('buku');
		}
	...
	public function kategori()
 	...
 	public function updatekategori()
 	{
 		...
 		$data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
 		$data['buku'] = $this->ModelBuku->kategoriWhere(['id' => $this->uri->segment(3)])->result_array();
 		...
 		if ($this->form_validation->run() == false) {
 			...
 			redirect('buku/kategori');
 			$this->session->set_flashdata('pesan', '<div class="alert-danger" role="alert">kategori Berhasil diupdate </div>');
 			redirect('buku/kategori'); 		
 	}
